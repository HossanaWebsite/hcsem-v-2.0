...
export default function BlogContentPage() {
    ...
    const [settings, setSettings] = useState({
        blogPageTitle: defaults.blogPageTitle,
        blogPageSubtitle: defaults.blogPageSubtitle,
        blogPageImage: defaults.blogPageImage,
        showBlogHeader: true
    });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success('Blog page settings saved successfully');
            } else {
    ...
    return (
        <div className="space-y-8 max-w-5xl mx-auto pb-20">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold font-heading text-white">Blog Page Content</h1>
                    <p className="text-slate-400">Manage blog page appearance</p>
                </div>
                <motion.button
            ...
            <section className={`glass-panel p-6 rounded-2xl border border-white/5 bg-slate-900/50 backdrop-blur-xl transition-all ${!settings.showBlogHeader ? 'opacity-50 grayscale-[0.5]' : ''}`}>
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-semibold text-white">Page Header</h2>
                    <div className="flex items-center gap-3">
                        <span className="text-sm text-slate-400">{settings.showBlogHeader ? 'Visible' : 'Hidden'}</span>
                        <button
                            onClick={() => setSettings({ ...settings, showBlogHeader: !settings.showBlogHeader })}
                            className={`w-12 h-6 rounded-full transition-colors relative ${settings.showBlogHeader ? 'bg-indigo-600' : 'bg-slate-700'}`}
                        >
                            <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${settings.showBlogHeader ? 'right-1' : 'left-1'}`} />
                        </button>
                    </div>
                </div>
                ...
                <div className="grid gap-8 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative aspect-video rounded-xl overflow-hidden border border-white/10 bg-slate-950 group">
                            ...
                            <div className={`absolute inset-0 bg-black/50 transition-opacity flex items-center justify-center ${settings.blogPageImage ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}>
                                <label className="cursor-pointer bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-lg backdrop-blur-sm transition-colors border border-white/10 flex items-center gap-2">
                                    <Upload className="w-4 h-4" /> {settings.blogPageImage ? 'Change Image' : 'Upload Image'}
